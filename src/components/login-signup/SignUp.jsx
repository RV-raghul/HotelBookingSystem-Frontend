...
function SignUp() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      apiService
        .post(apiRoutes.SIGNUP.url, { username, email, password })
        .then((res) => {
    ...
    <div className="bg-slate-200 flex justify-center items-center w-full min-h-screen p-4">
      <div className="bg-white rounded-2xl flex flex-col md:flex-row w-full md:w-[90vw] lg:w-[70vw] shadow-lg overflow-hidden">
        ...
        <div className="w-full md:w-2/3 p-6 flex flex-col justify-center items-center">
          <h1 className="text-3xl text-center font-extrabold mb-4 header">Sign Up</h1>
          <form onSubmit={handleSubmit} className="flex flex-col gap-2 w-full max-w-md">
            <label className="font-semibold text-gray-700 subheader">Username *</label>
            <input
              name="username"
              type="text"
            ...
            />
            <label className=" subheader font-semibold text-gray-700">Email ID *</label>
            <input
              name="email"
              type="text"
              ...
              className="p-2 h-10 border-2 rounded-sm"
            />
            <label className="font-semibold subheader text-gray-700">Password *</label>
            <input
              name="password"
              type="password"
            ...
            <button
              type="submit"
              className=" subheader mt-4 border-2 border-black h-10 rounded-sm bg-black text-white hover:bg-gray-800 transition"
            >
              Create Account
            </button>
          </form>
